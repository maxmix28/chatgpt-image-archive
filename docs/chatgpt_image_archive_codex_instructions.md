...
export type GeneratedImage = {
  ...
  width: number;
  height: number;
  fileType: string; // 保存後は原則 image/webp
  originalWidth?: number;
  originalHeight?: number;
  originalFileType?: string;
  hash: string;
  blob: Blob; // 最大1024px / WebP / 品質0.8に変換した保存用画像。元画像は保存しない
  thumbnailBlob: Blob; // 互換用。MVPでは blob と同じBlobでよい
...
画像登録時に、元画像を保存せず、保存用のWebP画像を生成してください。
...
WebP変換が失敗した場合は、保存に失敗したことをユーザーに表示してください。元画像はIndexedDBにもZIPにも保持しません。
...
thumbnails/
  generated/
    {imageId}.webp（互換用。images/generated/{imageId}.webp と同じファイルを指してよい）
  references/
    {referenceImageId}.webp（互換用。images/references/{referenceImageId}.webp と同じファイルを指してよい）
```
...
Blob自体はJSONに入れず、ファイルパスのみを保持してください。現行仕様では元画像ではなく、最大1024px / WebP / 品質0.8に変換した画像のみをZIPへ含めます。
...
- Settings
- 最大1024px / WebP / 品質0.8に変換した保存用画像ファイル
- thumbnailPath は互換用に出力してよいが、filePath と同じWebP画像を指してよい
