...
- グループ単位と画像単位のお気に入り
- 画像単位のステータス、評価、タグ、メモ
- 画像登録時の1024px WebP変換、サイズ取得、SHA-256ハッシュによる完全一致重複チェック
...
当初の推奨はReact + TypeScript + Vite + Dexie + JSZipでしたが、この作業環境では `npm` が利用できなかったため、依存追加なしで動く静的PWAとして実装しています。画像生成API連携は行いません。Supabase同期を有効にした場合のみ、ログイン中のSupabaseプロジェクトへ通信します。
...
8. 詳細画面からプロンプトコピー、WebP画像保存、編集、削除ができます。
9. `設定` からZIPエクスポート、追加インポート、全置換インポートを実行できます。
10. `設定` の `Supabase同期` でログインすると、PCとスマホで自動同期できます。
...
- Supabase以外のクラウド同期、Google Drive / OneDrive / Dropbox連携、複数ユーザー管理機能はありません。
- OCR、AIタグ付け、類似画像検索、画像編集、ゴミ箱機能はありません。
- Android共有メニューからの直接取り込みは未対応です。
- ZIPインポートは、このアプリが出力する無圧縮ZIPを対象にしています。
- GIFは静止画のWebPとして保存します。
- 元画像は保存しません。登録時に最大1024pxのWebPへ変換した画像だけを保存します。

## Supabase同期

Supabase同期を使う場合は、Supabase側で以下を用意します。

- `archive-images` bucket
- `prompt_groups`
- `generated_images`
- `reference_images`
- RLS policy
- authenticated roleへの権限付与
...
Supabaseの `SQL Editor` でこのSQLを実行してください。Storage bucketは先に作成しておきます。
...
- ステータス
- 評価
- 最大1024px / WebP / 品質0.8の画像

元画像はSupabaseにも保存しません。
...
画像は登録時に最大1024px、WebP、品質0.8へ変換し、その変換後画像だけをIndexedDBに保存します。Supabase同期を有効にした場合も、この変換後画像だけをアップロードします。元画像BlobはIndexedDB、Supabase、ZIPのいずれにも保存しません。ブラウザの閲覧データ削除、プロファイル削除、端末故障などで失われる可能性があるため、定期的にZIPエクスポートしてください。
...
`metadata.json` にはグループ、画像メタデータ、参考画像メタデータ、設定を保存します。Blob本体はJSONに入れず、ZIP内のWebPファイルパスとして参照します。互換性のため `thumbnailPath` も出力しますが、現在は `filePath` と同じWebPファイルを指します。
